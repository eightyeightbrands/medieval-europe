...
final class Kohana {
	...
	public static function exception_handler($exception, $message = NULL, $file = NULL, $line = NULL)
	{
		try
		{
			...
			//var_dump( $level . '-' . $error . '-' . $code ); exit;
			...
			{				
				...
				// gestione speciale memcache. se è giu, ignoriamo l'errore perchè
				// i dati vengono presi dal db. Il codice 8 di memcache 
				
				kohana::log('debug', "Error code: $code - message : $message" );			
				if ( $code == 8 and strpos( $message, 'Memcache' ) !== false )
					return;
				
				// gestione speciale vprintf
				if ( $code == 2 and strpos( $message, 'vsprintf' ) !== false )
				{
				
					$trace = array_slice(debug_backtrace(), 1);
					$trace = self::backtrace($trace);
					//kohana::log('info', '-> Offending string:' . $trace );
					return;
				}
	...
	public static function lang($key, $args = array())
	{
		...
		if ( ! isset(self::$internal_cache['language']['en_US'][$group]))
		{
			// Messages for this group
			$messages = array();

			if ($files = self::find_file('i18n', 'en_US'.'/'.$group))
			{
				foreach ($files as $file)
				{
					include $file;
			...
			self::$internal_cache['language']['en_US'][$group] = $messages;
		}
		
		// Get the line from cache
		$line = self::key_string(self::$internal_cache['language'][$locale], $key);
		// retry with english language
		if ($line === NULL)
			$line = self::key_string(self::$internal_cache['language']['en_US'], $key);
		if ($line === NULL)
		{
